	...
	public class ViewEditor : UnityEditor.Editor
	{
		...
		private Type[] GetComponentPrototypeTypes()
		{
			return AppDomain.CurrentDomain.GetAssemblies()
				...
				.Where(x => x.GetInterface(nameof(IEntityComponent)) != null)
		...
		public override void OnInspectorGUI()
		{
			...
			{
				var components = entityView.GetComponents<IEntityComponent>()
					.Where(x => x != null);
				
				var characterComponents = components as IEntityComponent[] ?? components.ToArray();
				
				{
					...
					if (GUI.Button(buttonRect, "+", EditorStyles.miniButton))
					{
						...
						EditorUtility.DisplayCustomMenu(buttonRect, availableComponents.Select(x => new GUIContent(x.Name)).ToArray(), -1,
							(_, _, selected) =>
							{
								var component = Undo.AddComponent(entityView.gameObject, availableComponents[selected]);
